...
import { Play, Download, Share2, Search, SlidersHorizontal, LayoutGrid, Save, Sparkles } from "lucide-react";
...
export default function DashboardScannerPage() {
  ...
  const [activeMode, setActiveMode] = useState<'magic' | 'visual'>('magic');
  ...
  return (
    <div className="p-4 md:p-8 bg-[#0D1117] min-h-full space-y-6">
      {/* Header */}
      <div className="flex flex-col md:flex-row md:justify-between md:items-end gap-4 border-b border-[#30363D]/40 pb-6">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-2 max-w-xl leading-relaxed">
            Scan and filter the Nifty universe in real-time using either natural language AI prompts or visual drag-and-drop node graph filters.
          </p>
        </div>

        {/* Mode Toggle Selector */}
        <div className="flex bg-[#161B22] p-1 rounded-lg border border-[#30363D] self-start md:self-end">
          <button 
            type="button"
          ...
          >
            <SlidersHorizontal size={14} className={activeMode === 'visual' ? 'text-[#388BFD]' : ''} />
            Visual Logic Graph
          </button>
        </div>
      </div>
